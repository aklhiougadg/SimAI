...
namespace MockNccl {
    ...
    */
    ...
    if(_TP_size>1){
      ...
      for(int i =0;i<TP_nums;i++){
        ...
        NVSwitchs.clear();
        for(int idx:TPnodes){
          if (idx < 0 || idx >= (int)_NVSwitch.size()) continue;
  ...
  std::shared_ptr<void> MockNcclGroup::getFlowModels(GroupType type , int rank, AstraSim::ComType op,uint64_t data_size,int layer_num,State loopstate){
    ...
    if(flow_models.count(flow_model_name)){
      ...
      // Switch: SIMAI_DUMP_DETAILED_FLOWS (default "1" = enabled, preserves existing behavior).
      ...
      static int dump_detailed = [](){
          const char* e = std::getenv("SIMAI_DUMP_DETAILED_FLOWS");
          if (!e) e = std::getenv("AS_DUMP_DETAILED_FLOWS");  // fallback legacy name
          return (e==nullptr || strcmp(e,"0")!=0) ? 1 : 0;
      }();
      if (dump_detailed) {
        // Obtain algo/proto info for CSV provenance columns
        struct ncclInfo* dump_info = get_algo_proto_info(type, rank, op, data_size);
        dumpDetailedFlowModels(flow_model_name, op, data_size, flow_models[flow_model_name], dump_info);
      }
  ...
  std::map<int,std::shared_ptr<FlowModels>> MockNcclGroup::genFlowModels(GroupType type , int rank, AstraSim::ComType op,uint64_t data_size){
    // v2.30: check if PAT algorithm was selected for AllGather/ReduceScatter.
    // get_algo_proto_info() already handles the nNodes>1 && data_size>=threshold logic.
    if (op == AstraSim::ComType::All_Gather || op == AstraSim::ComType::Reduce_Scatter) {
      struct ncclInfo* info = get_algo_proto_info(type, rank, op, data_size);
      if (info && info->algorithm == NCCL_ALGO_PAT) {
        return genPATFlowModels(type, rank, op, data_size);
      }
    }
    switch (op) {
  ...
  // v2.30 PAT (Parallel Aggregated Trees) FlowModel generation.
  // Uses full state machine from NCCL 2.30 (PatAGAlgorithm / PatRSAlgorithm)
  // to generate accurate binomial-tree communication patterns.
  // Each rank communicates with XOR-distance peers in log2(nRanks) phases.
  std::map<int,std::shared_ptr<FlowModels>> MockNcclGroup::genPATFlowModels(
      GroupType type, int rank, AstraSim::ComType op, uint64_t data_size) {
    FlowModels result = {};
    ...
    if (GroupIndex.count(std::make_pair(rank, type)) == 0) {
      NcclLog->writeLog(NcclLogLevel::ERROR, "No group info for genPATFlowModels.");
      return {};
    }
    gp_idx = GroupIndex[std::make_pair(rank, type)];
    ...
    // PAT parameters matching NCCL defaults
    int stepSize = 8 * 1024 * 1024;  // 8MB (NCCL_STEPS * stepDepth)
    int stepDepth = 4;               // NCCL_PAT_NWORKERS / WARP_SIZE
    size_t count = data_size / sizeof(float);  // element count
    size_t channelCount = count / nChannels;
    int chunkCount = (int)(channelCount / nranks);
    if (chunkCount <= 0) chunkCount = 1;

    // Generate flows for each channel
    for (int ch = 0; ch < nChannels; ch++) {
      // For each rank, run the PAT state machine
      for (int r = 0; r < nranks; r++) {
        int actual_rank = gp_info.Ranks[r];
        int step = 0;

        if (op == AstraSim::ComType::All_Gather) {
          PatAGAlgorithm<float> algo(stepSize, stepDepth, stepDepth,
                                     0, channelCount, count, chunkCount, r, nranks);
          while (true) {
            ncclPatStep ps = {};
            algo.getNextOp(&ps);
            bool skipped = (ps.flags & PatSkipped) != 0;
            if (!skipped) {
              // AG: rank sends to peer at sendDim distance
              if (ps.sendDim >= 0) {
                int sendPeerIdx = (r + (1 << ps.sendDim)) % nranks;
                int sendPeerRank = gp_info.Ranks[sendPeerIdx];
                uint64_t flowSize = data_size / nranks / nChannels;
                SingleFlow flow(g_flow_id, actual_rank, sendPeerRank, flowSize,
                               {}, {}, {}, ch, step, nranks - 1, "PAT");
                result[std::make_pair(ch, g_flow_id)] = flow;
                g_flow_id++;
              }
            }
            if (ps.last) break;
          ...
          // ReduceScatter
          PatRSAlgorithm<float> algo(stepSize, stepDepth, stepDepth,
                                     0, channelCount, count, chunkCount, r, nranks);
          while (true) {
            ncclPatStep ps = {};
            algo.getNextOp(&ps);
            bool skipped = (ps.flags & PatSkipped) != 0;
            if (!skipped) {
              // RS: rank sends to peer at sendDim distance (reverse direction)
              if (ps.sendDim >= 0) {
                int sendPeerIdx = (r + (1 << ps.sendDim)) % nranks;
                int sendPeerRank = gp_info.Ranks[sendPeerIdx];
                uint64_t flowSize = data_size / nranks / nChannels;
    ...
    // Build rank2flowmodels mapping
    for (auto& flow_it : result) {
      int src = flow_it.second.src;
      int dst = flow_it.second.dest;
      rank2flowmodels[src][std::make_pair(flow_it.first.first, flow_it.first.second)] = flow_it.second;
      rank2flowmodels[dst][std::make_pair(flow_it.first.first, flow_it.first.second)] = flow_it.second;
    }
    for (auto& it : rank2flowmodels) {
  ...
  // Broadcast flow generation (Ring-based, matching NCCL 2.30 behavior).
  // NCCL 2.30 tuning.cc L294 explicitly excludes Tree/NVLS/CollNet for Broadcast;
  // only Ring algorithm is used. The root rank sends data around the ring,
  // each rank forwards to the next until all ranks have received the data.
  // nSteps = nRanks - 1 (full ring traversal from root).
  std::map<int,std::shared_ptr<FlowModels>> MockNcclGroup::genBroadcastFlowModels(GroupType type, int rank, uint64_t data_size) {
    FlowModels result = {};
    std::map<int,FlowModels> rank2flowmodels;
    std::map<int,std::shared_ptr<FlowModels>> rank2pflowmodels;
    SingleFlow tmp_result;
    ...
    if (GroupIndex.count(std::make_pair(rank, type)) == 0) {
      NcclLog->writeLog(NcclLogLevel::ERROR, "No group info for genBroadcastFlowModels.");
      return {};
    }
    gp_idx = GroupIndex[std::make_pair(rank, type)];
    ringchannels = Allringchannels[gp_idx];
    ...
    int nranks = gp_info.nRanks;
    // Broadcast: root (rank 0) sends data_size to all others via ring.
    // Each step: rank[i] sends to rank[i+1] in the ring.
    // nSteps = nRanks - 1, chunkSize = data_size / nChannels
    int root = 0; // Default root = 0 (NCCL convention)

    for (auto it = ringchannels.begin(); it != ringchannels.end(); it++) {
      auto ring = it->second;
      auto ring_id = it->first;
      uint64_t chunksize = data_size / ringchannels.size();
      int chunkcount = nranks - 1;

      // Generate nRanks-1 sequential flows along the ring
      std::vector<int> ring_order;
      for (auto rank_it = ring.begin(); rank_it != ring.end(); rank_it++) {
        ring_order.push_back(rank_it->first);
      }

      int prev_flow_id = -1;
      for (int step = 0; step < nranks - 1; step++) {
        int src_rank = ring_order[step];
        int dst_rank = ring_order[(step + 1) % nranks];
        std::vector<int> prevranks;
        if (step > 0) {
          prevranks = {ring_order[(step - 1 + nranks) % nranks]};
        } else {
          // First step: root sends, prev is the ring predecessor of root
          prevranks = {ring_order[nranks - 1]};
        }
        std::vector<int> parent_flows;
        if (prev_flow_id >= 0) {
          parent_flows = {prev_flow_id};
        }

        tmp_result = SingleFlow(
            g_flow_id, src_rank, dst_rank, chunksize,
            prevranks, parent_flows, {},
            ring_id, step, chunkcount, "RING");

        if (prev_flow_id >= 0) {
          result[std::make_pair(ring_id, prev_flow_id)].child_flow_id.push_back(g_flow_id);
        }
        result[std::make_pair(ring_id, g_flow_id)] = tmp_result;
        prev_flow_id = g_flow_id;
        g_flow_id++;
      }
    }

    // Build rank2flowmodels
    for (auto flow_it = result.begin(); flow_it != result.end(); flow_it++) {
      int src = flow_it->second.src;
      int dst = flow_it->second.dest;
      rank2flowmodels[src][std::make_pair(flow_it->first.first, flow_it->first.second)] = flow_it->second;
      rank2flowmodels[dst][std::make_pair(flow_it->first.first, flow_it->first.second)] = flow_it->second;
  ...
  std::map<int,std::shared_ptr<FlowModels>> MockNcclGroup::genAllReduceFlowModels(GroupType type , int rank,uint64_t data_size){
    ...
    switch (ncc_info->algorithm) {
      case NCCL_ALGO_TREE: {
        // Tree flow generation: use genAllReduceTreeFlowModels() which returns
        // shared_ptr<FlowModels>. We need to wrap it into the expected map format.
        // Note: On H20x8 single-node, NCCL never selects Tree (always RING or NVLS).
        // This path is only reached via manual override (e.g., SIMAI_FORCE_ALGO=TREE).
        auto tree_flows = genAllReduceTreeFlowModels(type, rank, data_size);
        if (tree_flows && !tree_flows->empty()) {
          std::map<int,std::shared_ptr<FlowModels>> rank2pflowmodels;
          // Tree returns a single shared FlowModels for all ranks
          for (auto& flow_pair : *tree_flows) {
            int src = flow_pair.second.src;
            int dst = flow_pair.second.dest;
            if (rank2pflowmodels.find(src) == rank2pflowmodels.end())
              rank2pflowmodels[src] = std::make_shared<FlowModels>();
            if (rank2pflowmodels.find(dst) == rank2pflowmodels.end())
              rank2pflowmodels[dst] = std::make_shared<FlowModels>();
            (*rank2pflowmodels[src])[flow_pair.first] = flow_pair.second;
            (*rank2pflowmodels[dst])[flow_pair.first] = flow_pair.second;
          }
          return rank2pflowmodels;
        }
        // Fallback to Ring if Tree channels not available
  ...
  std::shared_ptr<FlowModels> MockNcclGroup::genAllReduceTreeFlowModels(GroupType type , int rank,uint64_t data_size){
    ...
    if(GroupIndex.count(std::make_pair(rank,type))==0){
      NcclLog->writeLog(NcclLogLevel::ERROR,"There is no corresponding group info, resulting in an error in genAllReduceTreeFlowModels.");
      return {};
    }
    gp_idx = GroupIndex[std::make_pair(rank,type)];
    if(Alltreechannels.count(gp_idx)==0){
      NcclLog->writeLog(NcclLogLevel::ERROR,"No tree channels available for gp_idx=%d, falling back.", gp_idx);
      return {};
    }
    gp_info = AllGroups[gp_idx];
  ...
  ncclInfo* MockNcclGroup::get_algo_proto_info(
    ...
    switch (op) {
      ...
      case AstraSim::ComType::All_Gather:
      case AstraSim::ComType::Reduce_Scatter:
          // v2.30 PAT (Parallel Aggregated Trees) selection.
          // PAT requires: nNodes > 1 AND nRanks == nNodes (1 GPU per node)
          // AND data within [PAT_MIN_BYTES, PAT_MAX_BYTES] range.
          // Source: nccl-2.30/src/graph/tuning.cc L220-223 (ncclPatEnable)
          //         nccl-2.30/src/graph/tuning.cc L429-434 (PAT latency model)
          // Real-machine observation (H20, 2x1GPU, NCCL 2.30.7):
          //   512KB -> PAT, 1MB+ -> RING (PAT only advantageous for small messages)
          {
            // SIMAI_PAT_ENABLE: 0=disable PAT, 1=force PAT, 2=auto (default)
            static const int pat_enable = []() {
                const char* e = std::getenv("SIMAI_PAT_ENABLE");
                return e ? atoi(e) : 2;
            }();
            static const uint64_t PAT_MIN_BYTES = []() {
                const char* e = std::getenv("SIMAI_PAT_MIN_BYTES");
                return e ? strtoull(e, nullptr, 10) : 524288ULL; // 512KB default
            }();
            static const uint64_t PAT_MAX_BYTES = []() {
                const char* e = std::getenv("SIMAI_PAT_MAX_BYTES");
                return e ? strtoull(e, nullptr, 10) : 1048576ULL; // 1MB default
            }();

            int nNodes = gp_info.nNodes;
            int nRanks = gp_info.nRanks;
            bool pat_eligible = (nNodes > 1 && nRanks == nNodes);

            if (pat_enable == 0) {
                // PAT disabled by user
                info->algorithm = NCCL_ALGO_RING;
            } else if (pat_enable == 1 && pat_eligible) {
                // Force PAT regardless of size threshold
                info->algorithm = NCCL_ALGO_PAT;
            } else if (pat_enable == 2 && pat_eligible) {
                // Auto mode: NCCL cost model approximation with dual-threshold fallback.
                // NCCL PAT latency model (tuning.cc L429-434):
                //   lat_PAT = baseLat(8.0) + log2(nNodes) * (interLat/3.5) + nRanks*2.8
                // NCCL Ring latency model (tuning.cc L393-412, simplified for ppn=1):
                //   lat_Ring = baseLat(8.4) + (nRanks-1) * interLat
                // Where interLat = hwLat[NET][TREE][Simple] = 14.0 (for ppn=1 scenario)
                // PAT wins on latency but loses on bandwidth (0.75x).
                // For large messages, bandwidth dominates -> RING wins.
                // Dual-threshold approximation matches real-machine observation:
                //   PAT selected when PAT_MIN_BYTES <= data_size <= PAT_MAX_BYTES
                if (data_size >= PAT_MIN_BYTES && data_size <= PAT_MAX_BYTES) {
                    info->algorithm = NCCL_ALGO_PAT;
                } else {
                    info->algorithm = NCCL_ALGO_RING;
                }
    ...
    // consumed downstream (dead field). Under the SIMAI_PROTO_AWARE switch (default
    // "1" for v2.30 mock = ON, enabling protocol-aware FlowModel provenance)
    ...
    // SIMAI_FORCE_PROTO: override protocol selection for testing/Cartesian-product.
    // Values: "LL", "LL128", "Simple". If set, bypasses auto-selection.
    static const int forced_proto = [](){
        const char* e = std::getenv("SIMAI_FORCE_PROTO");
        if (!e) return -1;
        if (strcmp(e, "LL") == 0) return NCCL_PROTO_LL;
        if (strcmp(e, "LL128") == 0) return NCCL_PROTO_LL128;
        if (strcmp(e, "Simple") == 0) return NCCL_PROTO_SIMPLE;
        return -1; // invalid value, fall through to auto
    }();
    if (forced_proto >= 0 && info->algorithm != NCCL_ALGO_PAT) {
      // SIMAI_FORCE_PROTO applies to non-PAT algorithms only.
      // PAT is always Simple (NCCL 2.30 tuning.cc L302 skips non-Simple for PAT).
      info->protocol = forced_proto;
    } else if (info->algorithm == NCCL_ALGO_PAT) {
      // PAT: force Simple protocol unconditionally (matches NCCL 2.30 behavior)
      info->protocol = NCCL_PROTO_SIMPLE;
    } else {
      static const int proto_aware = [](){
          const char* e = std::getenv("SIMAI_PROTO_AWARE");
          if (!e) e = std::getenv("AS_PROTO_AWARE");  // fallback legacy name
          // v2.30 default: ON (protocol-aware); set to "0" to revert to UNDEF
          return (e && strcmp(e,"0")==0) ? 0 : 1;
      }();
      if (!proto_aware) {
  ...
  void MockNcclGroup::dumpDetailedFlowModels(
    ...
    if (!detailed_dump_header_written) {
      ...
      if (ofs.is_open()) {
        ofs << "collective,op,data_size,algorithm,protocol,channel_id,flow_id,src,dest,flow_size,"
    ...
    int algo = info ? info->algorithm : -1;
    int proto = info ? info->protocol : -1;
